################################################################################
# Calculation of the evidence of the flat LCDM and Risaliti Lusso fitted model
# flat LCDM -> prior = the posterior on 15 SS
# RL -> prior = posterior from QSA +JLA data = a2, a3 figure4 RL paper 
...
def Ell_DL_LCDM(z, om, h):
    Omega_m =om
    s = np.power( (1.0-Omega_m)/Omega_m , 1.0/3.0 )
    HH = h/2997.9
    return ((1.0+z)/(HH * np.sqrt(s * Omega_m)) )*(T(s) - T(s/(1.0+z)) )
...
###############################################
data_z_D_dD = pd.read_csv("../BIGcat_final_101.dat" ,sep="\s+").values    
...
samp = np.load("SSdatasets/median_dataset_15SS.dat.npy")

mean = np.mean(samp, axis=0)
cov = np.cov(samp, rowvar=0)

print(mean, cov)
rv_LCDM = stats.multivariate_normal(mean, cov)


# prior of the LCDM
def prior_Omega(om, h):
    params = om,h
    return rv_LCDM.pdf(params)
print("check normalization = ", dblquad(prior_Omega, 0.00000001,0.99999999, 0.1, 1)[0])

# Product of the likelihood and prior
def likelihood_x_Omega(om, h, z, D, dD):
    # likelihood with given redshifts and distance luminosities
    # beta prior between 0 and 1
    return (np.product( np.exp(-(D - Ell_DL_LCDM(z, om, h))**2 /(2*(dD*dD) ))/(np.sqrt(2* np.pi)*dD) ))*prior_Omega(om ,h)

# evidence of the LCDM
def evidence_LCDM(z,D,dD):
    return dblquad(likelihood_x_Omega, 0.00000001,0.99999999, 0.1, 1, args=(z,D,dD))[0]

############################################################
# Risaliti Lusso Model based on 
############################################################
...
print("check the ratio" ,dblquad(likelihood_x_a2_a3, -5., +5., -5., +5.,  args=(z_SS[:Ncheck],DL_SS[:Ncheck],dDL_SS[:Ncheck]),epsabs=1e-50)[0]/ evidence_a2_a3(z_SS[:Ncheck],DL_SS[:Ncheck],dDL_SS[:Ncheck]) )
...
def Bayes_factor_LCDM_RL(z, D, dD):
    return evidence_LCDM(z,D,dD)/evidence_a2_a3(z,D, dD)
...
# create file
file = h5py.File('EvidenceData_post.h5','w')
